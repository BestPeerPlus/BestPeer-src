...
/**
 * This class is used to generate sample data for the ERP database. 
 */
...
public class GenerateLocalDatabaseSample 
{
	...
	public void ConnectToDatabase()
	{
		...
		while(count < 60)
		{
			try
			{
				...
				driver = dbProperty.localdb_driver;
				url = dbProperty.localdb_url;
				username = dbProperty.localdb_username;
				port = dbProperty.localdb_port;
				password = dbProperty.bestpeerdb_password;
				localDB = dbProperty.localdb_dbName;
				
				Class.forName(driver);
				conn = DriverManager.getConnection(url+localDB, username, password);
				isConnected = true;
				return;
			}
			catch(Exception e)
	...
	public void ExecuteSQLCommand()
	{
		...
		try
		{
			BufferedReader reader = new BufferedReader(new FileReader("localdbsample.txt"));
	...
	{   
		GenerateLocalDatabaseSample app = 
			new GenerateLocalDatabaseSample();
